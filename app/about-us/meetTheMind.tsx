...
import { MeetTheMindSkeleton } from "@/components/skeletons";
...
const MeetTheMind = () => {
  ...
  // Effect to manage body overflow when popup is open
  useEffect(() => {
    if (isPopupOpen) {
  ...
  const meetOurTeamBlock = cachedData || freshData;
  ...
  useEffect(() => {
    if (data) {
      ...
      if (meetOurTeamData) {
        dispatch(setMeetOurTeamData(meetOurTeamData));
      }
    }
  }, [data, dispatch]);
  ...
  if (
    !meetOurTeamBlock ||
    !meetOurTeamBlock.teamMember ||
    !meetOurTeamBlock.teamMember.nodes ||
    meetOurTeamBlock.teamMember.nodes.length === 0 ||
    !meetOurTeamBlock.meetEveryCsian
  ) {
    return <MeetTheMindSkeleton />;
  }
  ...
  return (
    <>
      <section className="meet-the-mind bg-black py-[120px] pt-[60px]">
        <div className="container max-w-[1440px] px-[20px] mx-auto">
          <div className="flex flex-col items-center justify-center gap-[60px]">
            <h2 className="h2 text-white text-center max-w-[1045px] normal-case">
              {meetOurTeamTitle}
            </h2>
            <div className="grid 2xl:grid-cols-4 xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-3 sm:grid-cols-1 grid-cols-1 gap-[30px] w-full 2xl:pb-[100px] xl:pb-[100px] lg:pb-0 md:pb-0 sm:pb-0 pb-0">
              {teamMember?.nodes?.map((member: any, idx: number) => (
                <div
                  key={idx}
                  className={`group relative 2xl:h-[430px] xl:h-[430px] lg:h-[430px] md:h-[430px] sm:h-full h-full 2xl:aspect-auto xl:aspect-auto lg:aspect-auto md:aspect-auto sm:aspect-square aspect-square rounded-xl overflow-hidden transition-all duration-500 py-[20px] px-[10px] flex items-end cursor-pointer ${idx % 2 === 1
                ...
                >
                  ...
                  {/* Content Area */}
                  <div className="bg-white rounded-[12px] p-[15px] relative w-full z-10 flex items-center gap-[10px] flex-row flex-wrap">
                    ...
                    <p className="text-[#707070] font-lato text-[15px] leading-[26px] transition-all duration-500">
        ...
        >
          ...
          <div className="relative flex items-center justify-center min-h-screen p-[20px]">
            ...
            <div className="bg-[#292929] max-h-[calc(100vh-80px)] overflow-y-auto rounded-[20px] shadow-[0px_24px_54px_0px_rgba(0,0,0,0.65)] w-full max-w-[1400px] xl:h-[744px] xl:h-[auto] md:h-auto sm:h-auto h-auto relative overflow-hidden custom-scrollbar sm:pt-[20px] md:pt-[20px] lg:pt-0 ">
              ...
              <button
                onClick={closePopup}
                className="absolute xl:top-[40px] xl:right-[40px] top-[10px] right-[10px] z-20 w-[43.84px] h-[43.84px] rounded-full flex items-center justify-center opacity-75 hover:opacity-100 transition-opacity duration-300 group"
              >
                <svg
                  width="44"
                  height="44"
