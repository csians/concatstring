...
import { WorkingMethodSkeleton } from "@/components/skeletons";
...
const WorkingMethod = () => {
  ...
  useEffect(() => {
    ...
    if (!path) {
      console.warn("Red path ref is not set");
      return;
    }
    if (!scrollContainer) {
    ...
    (path.style as any).strokeDashoffset = totalLength; // Start hidden

    const handleScroll = () => {
      const scrollTop = scrollContainer.scrollTop;
      const scrollHeight = scrollContainer.scrollHeight - scrollContainer.clientHeight;
      const scrollProgress = Math.min(scrollTop / scrollHeight, 1); // Ensure max 1
  ...
  return (
    <>
      <section className="working-method bg-black pt-[100px] 2xl:pb-[60px] xl:pb-[60px] lg:pb-[60px] md:pb-0 sm:pb-0 pb-0">
        <div className="container max-w-[1439px] px-[20px] mx-auto">
          <div className="relative working-method-wrap flex 2xl:gap-[130px] xl:gap-[130px] lg:gap-[30px] md:gap-[30px] sm:gap-[30px] gap-[30px] items-start 2xl:flex-row xl:flex-row lg:flex-col md:flex-col sm:flex-col flex-col">
            {/* <svg
            className="vector-path absolute top-[7%] left-[5%] h-[726px] z-0 pointer-events-none 2xl:block xl:block lg:hidden md:hidden sm:hidden hidden w-[50%]"
            xmlns="http://www.w3.org/2000/svg"
            width="684"
            height="726"
            viewBox="0 0 684 726"
          ...
          {/* New simple red path from top to bottom */}
          <svg
            className="vector-path vector-path-red absolute top-[-10%] xl:top-[0%] left-[4%] xl:left-[6%] 2xl:h-[110%] xl:h-[110%] lg:h-[100%] md:h-[100%] sm:h-[100%] h-[110%] z-10 pointer-events-none 2xl:block xl:block lg:hidden md:hidden sm:hidden hidden w-[50%]"
            xmlns="http://www.w3.org/2000/svg"
            width="684"
            height="726"
            viewBox="0 0 684 726"
          ...
          >
            <path
              ref={pathRef}
              id="method-path-red"
              d="M678.505 2.80078C678.505 4.27354 679.699 5.46745 681.171 5.46745C682.644 5.46745 683.838 4.27354 683.838 2.80078C683.838 1.32802 682.644 0.134115 681.171 0.134115C679.699 0.134115 678.505 1.32802 678.505 2.80078C680.967 2.34443 680.966 2.34465 680.966 2.34504C680.965 2.3454 680.964 2.34597 680.962 2.34669C680.959 2.34814 680.954 2.35028 680.948 2.35311C680.935 2.35876 680.916 2.36717 680.892 2.37833C680.842 2.40064 680.769 2.43391 680.672 2.47802C680.479 2.56624 680.193 2.69779 679.82 2.87161C679.074 3.21925 677.982 3.73599 676.591 4.41332C673.81 5.76796 669.84 7.76514 665.075 10.3368C655.547 15.4796 642.84 22.923 630.13 32.1233C617.423 41.3215 604.695 52.2882 595.14 64.4816C585.587 76.6728 579.175 90.1297 579.175 104.297C579.175 118.495 585.615 128.474 595.218 136.308C604.794 144.122 617.548 149.828 630.219 155.501C642.922 161.189 655.543 166.843 664.996 174.556C674.424 182.249 680.671 191.963 680.671 205.793C680.671 219.623 674.424 229.338 664.996 237.03C655.543 244.743 642.922 250.397 630.219 256.085C617.548 261.758 604.794 267.464 595.218 275.278C585.615 283.113 579.175 293.092 579.175 307.289C579.175 321.487 585.615 331.466 595.218 339.301C604.794 347.115 617.548 352.821 630.219 358.494C642.922 364.181 655.543 369.835 664.996 377.549C674.424 385.241 680.671 394.956 680.671 408.786C680.671 422.686 675.578 438.239 667.234 454.347C658.893 470.45 647.322 487.07 634.415 503.093C608.6 535.14 577.471 564.758 556.234 583.018C503.894 628.026 422.368 676.967 340.584 703.716C258.765 730.477 176.876 734.968 123.629 691.37C70.5564 647.913 39.9271 585.837 22.5663 534.588C13.8876 508.968 8.52884 486.066 5.34002 469.571C3.74566 461.323 2.6939 454.678 2.04086 450.096C1.71435 447.805 1.48752 446.029 1.3424 444.828C1.26985 444.227 1.21771 443.77 1.18376 443.463C1.16678 443.309 1.15435 443.193 1.14618 443.116C1.1421 443.078 1.13908 443.049 1.13709 443.029C1.1361 443.02 1.13536 443.013 1.13488 443.008C1.13464 443.006 1.13447 443.004 1.13435 443.003C1.13424 443.002 1.13419 443.001 0.636719 443.051C0.13925 443.102 0.139327 443.102 0.139466 443.104C0.139597 443.105 0.139799 443.107 0.14006 443.11C0.140583 443.115 0.141359 443.122 0.142393 443.132C0.144461 443.152 0.147562 443.182 0.15173 443.221C0.160065 443.3 0.17267 443.418 0.189826 443.573C0.224137 443.883 0.276651 444.343 0.349618 444.948C0.495551 446.156 0.723299 447.938 1.05087 450.237C1.70599 454.834 2.76042 461.495 4.3582 469.761C7.55364 486.291 12.9229 509.237 21.6192 534.909C39.0084 586.242 69.7171 648.518 122.996 692.143"
              stroke="#54A3DA"
              strokeWidth="4"
              fill="none"
            />
            ...
            {/* <svg
              ...
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              className="vector-path absolute top-[7%] left-[5%] h-full z-0 pointer-events-none 2xl:block xl:block lg:hidden md:hidden sm:hidden hidden w-[50%]"
            >
              <path
                ref={pathRef}
                id="method-path"
            ...
            <div className="relative flex flex-col gap-[60px] items-start 2xl:w-[50%] xl:w-[50%] lg:w-full md:w-full sm:w-full w-full z-[111]">
            ...
            >
              <div className="flex flex-col gap-[10px] me-[5px]">
                {workingCycle.map((step: any, idx: number) => (
                  <div
                    key={idx}
                    className={`flex flex-col items-start gap-[6px] py-[36px]${idx !== workingCycle.length - 1
